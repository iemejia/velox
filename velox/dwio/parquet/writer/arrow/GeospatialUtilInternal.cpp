...
#include "velox/dwio/parquet/writer/arrow/GeospatialUtilInternal.h"
...
/// byte (0x01 or 0x00), followed by geometry type + dimensions encoded as a
/// (uint32_t), followed by geometry-specific data. Coordinate sequences are
/// represented by a uint32_t (the number of coordinates) plus a sequence of
/// doubles (number of coordinates multiplied by the number of dimensions).
class WKBBuffer {
 public:
  ...
  void ReadCoords(uint32_t n_coords, bool swap, Visit&& visit) {
    ...
    if (size_ < total_bytes) {
      throw ParquetException(
          "Can't read coordinate sequence of ",
          total_bytes,
          " bytes from WKBBuffer with ",
          size_,
          " remaining");
    }
...
GeometryTypeAndDimensions ParseGeometryType(uint32_t wkb_geometry_type) {
  ...
  return {
      static_cast<GeometryType>(geometry_type_component),
...
void WKBGeometryBounder::MergeGeometry(std::string_view bytes_wkb) {
  MergeGeometry(
      ::arrow::util::span(
          reinterpret_cast<const uint8_t*>(bytes_wkb.data()),
          bytes_wkb.size()));
}

void WKBGeometryBounder::MergeGeometry(
    ::arrow::util::span<const uint8_t> bytes_wkb) {
...
void WKBGeometryBounder::MergeGeometryInternal(
    WKBBuffer* src,
  ...
  switch (geometry_type) {
    ...
    // dimensions. For the purposes of bounding, this does not cause us
    // problems. We pass record_wkb_type = false because we do not want the
    // child geometry to be added to the geometry_types list (e.g., for a
    // MultiPoint, we only want the code for MultiPoint to be added, not the
    // code for Point).
...
void WKBGeometryBounder::MergeSequence(
    WKBBuffer* src,
