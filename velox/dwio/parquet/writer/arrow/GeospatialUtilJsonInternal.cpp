...
#include "velox/dwio/parquet/writer/arrow/GeospatialUtilJsonInternal.h"
...
::arrow::Result<std::string> GeospatialGeoArrowCrsToParquetCrs(
  ...
  if (json_crs.IsString() &&
    ...
    // and return an empty crs to maximize compatibility with readers that do
    // not implement CRS support. PROJJSON stores this in the "id" member like:
    // {..., "id": {"authority": "...", "code": "..."}}
      ...
      if (identifier.HasMember("authority") && identifier.HasMember("code")) {
        if (identifier["authority"] == "OGC" && identifier["code"] == "CRS84") {
          return "";
            ...
            identifier["authority"] == "EPSG" && identifier["code"] == "4326") {
          return "";
        } else if (
            identifier["authority"] == "EPSG" && identifier["code"].IsInt() &&
  ...
  // the LogicalType crs (being sure to unescape a JSON string into a regular
  // string)
  if (json_crs.IsString()) {
...
::arrow::Result<std::string> MakeGeoArrowCrsMetadata(
  ...
  if (crs.empty()) {
    ...
    if (metadata && metadata->Contains(metadata_field)) {
      ARROW_ASSIGN_OR_RAISE(
          std::string projjson_value, metadata->Get(metadata_field));
      // This value should be valid JSON, but if it is not, we escape it as a
      // string such that it can be inspected by the consumer of GeoArrow.
  ...
  // JSON, insert it directly into GeoArrow's "crs" field. Otherwise, escape it
  // and pass it as a string value.
...
LogicalTypeFromGeoArrowMetadata(std::string_view serialized_data) {
  ...
  // most likely intent here (that the user meant to use the default Parquet
  // CRS)
  ...
  namespace rj = ::arrow::rapidjson;
  rj::Document document;
  if (document.Parse(serialized_data.data(), serialized_data.length())
          .HasParseError()) {
    return ::arrow::Status::Invalid(
        "Invalid serialized JSON data: ", serialized_data);
  }

  ARROW_ASSIGN_OR_RAISE(
      std::string crs, GeospatialGeoArrowCrsToParquetCrs(document));
  ...
  if (document.HasMember("edges") && document["edges"] == "planar") {
    ...
    return LogicalType::geography(
        crs, LogicalType::EdgeInterpolationAlgorithm::kSpherical);
  } else if (document.HasMember("edges")) {
    return ::arrow::Status::Invalid(
        "Unsupported GeoArrow edge type: ", serialized_data);
  }
...
::arrow::Result<std::shared_ptr<::arrow::DataType>> GeoArrowTypeFromLogicalType(
  ...
  if (logical_type.isGeometry()) {
    const auto& geospatial_type =
        ::arrow::internal::checked_cast<const GeometryLogicalType&>(
            logical_type);
    ARROW_ASSIGN_OR_RAISE(
  ...
  } else if (logical_type.isGeography()) {
    const auto& geospatial_type =
        ::arrow::internal::checked_cast<const GeographyLogicalType&>(
            logical_type);
    ARROW_ASSIGN_OR_RAISE(
        std::string crs_metadata,
        MakeGeoArrowCrsMetadata(geospatial_type.crs(), metadata));
    std::string edges_metadata =
        R"("edges": ")" + std::string(geospatial_type.algorithmName()) + R"(")";
    std::string serialized_data =
        std::string("{") + crs_metadata + ", " + edges_metadata + "}";
    return maybe_geoarrow_wkb->Deserialize(storage_type, serialized_data);
  } else {
    throw ParquetException(
        "Can't export logical type ", logical_type.toString(), " as GeoArrow");
  }
}
