...
/// modifications. Each chunk defines a new parquet data page which is
/// contiguously written out to the file. Since each page compressed
/// independently, the files' contents would look like the following with unique
/// page identifiers:
///
...
/// (CAS) system which splits the bytes stream into content defined blobs. The
/// CAS system will calculate a unique identifier for each blob, then store the
/// blob in a key-value store. If the same blob is encountered again, the system
/// can refer to the hash instead of physically storing the blob again. In the
/// example above, the CAS system would store Page1, Page2, Page3, and Page4
/// only once and the required metadata to reassemble the files. While the
/// deduplication is performed by the CAS system, the parquet chunker makes it
...
/// reader doesn't need to know about it. Each parquet column writer holds a
/// ContentDefinedChunker instance depending on the writer's properties. The
/// chunker's state is maintained across the entire column without being reset
/// between pages and row groups.
///
/// The chunker receives the record shredded column data (def_levels,
/// rep_levels, values) and goes over the (def_level, rep_level, value) triplets
/// one by one while adjusting the column-global rolling hash based on the
/// triplet. Whenever the rolling hash matches a predefined mask, the chunker
/// creates a new chunk. The chunker returns a vector of Chunk objects that
/// represent the boundaries of the chunks. Note that the boundaries are
/// deterministically calculated exclusively based on the data itself, so the
/// same data will always produce the same chunks - given the same chunker
/// configuration.
///
/// References:
...
class PARQUET_EXPORT ContentDefinedChunker {
 public:
  ...
  ///   chunk. Note that all data sent through the hash function is counted
  ///   towards the chunk size, including definition and repetition levels if
  ///   present.
  /// @param max_chunk_size Maximum chunk size in bytes
  ///   The chunker creates a new chunk whenever the chunk size exceeds this
  ///   value. The chunk size distribution approximates a normal distribution
  ///   between min_chunk_size and max_chunk_size. Note that the parquet writer
  ///   has a related `data_pagesize`
  ...
  ///   doesn't affect the chunking effectiveness, it results in more small
  ///   parquet data pages.
  /// @param norm_level Normalization level to center the chunk size around the
  /// average
  ...
  ///   the number of small chunks resulting in many small parquet data pages.
  ///   The default value provides a good balance between deduplication ratio
  ///   and fragmentation. Use norm_level=1 or norm_level=2 to reach a higher
  ///   deduplication ratio at the expense of fragmentation.
 ...
 private:
  /// @brief Get the rolling hash mask used to determine chunk boundaries, used
  /// for testing the mask calculation.
