...
// Arrow moved into the non-installed internal header arrow/util/spaced_internal.h
// after the version this writer was adapted from. The compaction/expansion is
// re-expressed on top of Arrow's still-public SetBitRunReader primitives.
...
/// Relocates values in buffer into the positions of non-null values as indicated
/// by a validity bitmap. Returns the number of values expanded, including nulls.
template <typename T>
inline int spacedExpand(
  ...
  std::memset(
      static_cast<void*>(buffer + idxDecode), 0, nullCount * sizeof(T));
  if (idxDecode == 0) {
