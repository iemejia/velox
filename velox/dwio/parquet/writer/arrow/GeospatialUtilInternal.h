...
/// integer geometry type (i.e., the value of geometry_type // 1000).
...
/// for the purposes of accumulating this type of bounds.
///
/// This class will ignore any NaN values it visits via UpdateXY[Z[M]](). This
/// is consistent with GEOS and ensures that ranges are accumulated in the same
/// way that other statistics in Parquet are accumulated (e.g., statistics of a
/// double column will return a min/max of all non-NaN values). In WKB
/// specifically, POINT EMPTY is represented by convention as all ordinate
/// values filled with NaN, so this behaviour allows for no special-casing of
/// POINT EMPTY in the WKB reader.
///
/// This class will propagate any NaN values (per dimension) that were
/// explicitly specified via setting mins/maxes directly or by merging another
/// BoundingBox that contained NaN values. This definition ensures that NaN
/// bounds obtained via EncodedGeoStatistics (which may have been written by
/// some other writer that generated NaNs, either on purpose or by accident) are
...
struct PARQUET_EXPORT BoundingBox {
  ...
  BoundingBox()
      : min{kInf, kInf, kInf, kInf}, max{-kInf, -kInf, -kInf, -kInf} {}
  ...
  void Merge(const BoundingBox& other) {
    for (int i = 0; i < 4; i++) {
      if (std::isnan(min[i]) || std::isnan(max[i]) ||
          std::isnan(other.min[i]) || std::isnan(other.max[i])) {
