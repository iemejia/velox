 ...
 public:
  ...
  void Update(const ByteArray* values, int64_t num_values) {
    ...
    if (is_wraparound_x()) {
      throw ParquetException(
          "Wraparound X is not suppored by GeoStatistics::Update()");
    }

    for (int64_t i = 0; i < num_values; i++) {
      const ByteArray& item = values[i];
      try {
        bounder_.MergeGeometry(
  ...
  void Update(const ::arrow::Array& values) {
    ...
    switch (values.type_id()) {
      ...
      default:
        throw ParquetException(
            "Unsupported Array type in GeoStatistics::Update(Array): ",
  ...
  void Decode(const EncodedGeoStatistics& encoded) {
    ...
    has_geometry_types_ =
        has_geometry_types_ && encoded.geospatial_types_present();
  }
...
void GeoStatistics::UpdateSpaced(
    const ByteArray* values,
    const uint8_t* valid_bits,
    int64_t valid_bits_offset,
    int64_t num_spaced_values,
    int64_t num_values) {
  impl_->UpdateSpaced(
      values, valid_bits, valid_bits_offset, num_spaced_values, num_values);
}
...
void GeoStatistics::Reset() {
  impl_->Reset();
}

bool GeoStatistics::is_valid() const {
...
std::array<double, 4> GeoStatistics::lower_bound() const {
  return impl_->lower_bound();
}

std::array<double, 4> GeoStatistics::upper_bound() const {
