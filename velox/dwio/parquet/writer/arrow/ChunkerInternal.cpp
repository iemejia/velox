...
#include "velox/dwio/parquet/writer/arrow/ChunkerInternalGenerated.h"
#include "velox/dwio/parquet/writer/arrow/Exception.h"

namespace facebook::velox::parquet::arrow::internal {

using facebook::velox::parquet::LevelInfo;

using ::arrow::internal::checked_cast;

static_assert(
    std::size(kGearhashTable) == kNumGearhashTables,
    "should update CDC code to reflect number of generated hash tables");
static_assert(
    sizeof(kGearhashTable) == kNumGearhashTables * 256 * 8,
    "each table should have 256 entries of 64 bit values");

namespace {

/// Calculate the mask to use for the rolling hash, the mask is used to
/// determine if a new chunk should be created based on the rolling hash value.
/// The mask is calculated based on the min_chunk_size, max_chunk_size and
/// norm_level parameters.
///
/// Assuming that the gear hash hash random values with a uniform distribution,
/// then each bit in the actual value of rolling_hash_ has even probability of
/// being set so a mask with the top N bits set has a probability of 1/2^N of
/// matching the rolling hash. This is the judgment criteria for the original
/// gear hash based content-defined chunking. The main drawback of this approach
/// is the non-uniform distribution of the chunk sizes.
///
...
/// distribution. Instead of using two different masks (one with a lower and one
/// with a higher probability of matching and switching them based on the actual
/// chunk size), we rather use 8 different gear hash tables and require having 8
/// consecutive matches while switching between the used hashtables. This
/// approach is based on central limit theorem and approximates normal
...
uint64_t
CalculateMask(int64_t min_chunk_size, int64_t max_chunk_size, int norm_level) {
  if (min_chunk_size < 0) {
  ...
  // need to target a smaller chunk size to reach the average size after
  // skipping the first `min_chunk_size` bytes; also divide by the number of
  // gearhash tables to have a a more gaussian-like distribution
  ...
  // the mask by taking the floor(log2(target_size))
  int mask_bits =
      std::max(0, ::arrow::bit_util::NumRequiredBits(target_size) - 1);

  // a user defined `norm_level` can be used to adjust the mask size, hence the
  // matching probability, by increasing the norm_level we increase the
  // probability of matching the mask, forcing the distribution closer to the
  // average size; norm_level is 0 by default
...
class ContentDefinedChunker::Impl {
 public:
  Impl(
      ...
      : level_info_(level_info),
        ...
        rolling_hash_mask_(
            CalculateMask(min_chunk_size, max_chunk_size, norm_level)) {}

  uint64_t GetRollingHashMask() const {
    return rolling_hash_mask_;
  }

  void Roll(bool value) {
    // Update the rolling hash with a boolean value, set has_matched_ to true if
    // the hash matches the
    if (++chunk_size_ < min_chunk_size_) {
      // short-circuit if we haven't reached the minimum chunk size, this speeds
      // up the chunking process since the gearhash doesn't need to be updated
      return;
    }
  ...
  void Roll(const uint8_t* value) {
    ...
    // has_matched_ to true if the hash matches the mask.
    ...
    for (size_t i = 0; i < kByteWidth; ++i) {
      ...
      has_matched_ =
          has_matched_ || ((rolling_hash_ & rolling_hash_mask_) == 0);
    }
  }
  ...
  bool NeedNewChunk() {
    ...
    // has_matched_ is set to true if we encountered a match since the last
    // NeedNewChunk() call
    if (ARROW_PREDICT_FALSE(has_matched_)) {
      has_matched_ = false;
      // in order to have a normal distribution of chunk sizes, we only create a
      // new chunk if the adjused mask matches the rolling hash 8 times in a
      // row, each run uses a different gearhash table (gearhash's chunk size
      // has geometric distribution, and we use central limit theorem to
      // approximate normal distribution, see section 6.2.1 in paper
      ...
      if (ARROW_PREDICT_FALSE(++nth_run_ >= kNumGearhashTables)) {
        ...
        // anywhere else in the code, in practice this doesn't seem to affect
    ...
    if (ARROW_PREDICT_FALSE(chunk_size_ >= max_chunk_size_)) {
      ...
      // reset the rolling hash state here, so the next NeedNewChunk() call will
  ...
  std::vector<Chunk> Calculate(
    ...
    // reset between pages and row groups. This enables that the chunking
    // process can be continued between different WriteArrow calls.
    //
    // Below we go over the (def_level, rep_level, value) triplets one by one
    // while adjusting the column-global rolling hash based on the triplet.
    // Whenever the rolling hash matches a predefined mask it sets the
    // `has_matched_` flag to true.
    //
    // After each triplet NeedNewChunk() is called to evaluate if we need to
    // create a new chunk. If the rolling hash matches the mask
    // `kNumGearhashTables` times in row (required for better chunk size
    // distribution) and satisfies the chunk size requirements, we create a new
    // chunk. See the `NeedNewChunk()` method for more details.
    std::vector<Chunk> chunks;
    int64_t offset;
    int64_t prev_offset = 0;
    int64_t prev_value_offset = 0;
    bool has_def_levels = level_info_.defLevel > 0;
    bool has_rep_levels = level_info_.repLevel > 0;
    ...
    if (!has_rep_levels && !has_def_levels) {
      ...
      for (int64_t offset = 0; offset < num_levels; ++offset) {
        ...
        Roll(&def_level);
        if (def_level == level_info_.defLevel) {
          RollValue(offset);
        }
        if (NeedNewChunk()) {
          chunks.push_back({prev_offset, prev_offset, offset - prev_offset});
      ...
      for (offset = 0; offset < num_levels; ++offset) {
        ...
        if (rep_level == 0 && NeedNewChunk()) {
          // if we are at a record boundary and need a new chunk, we create a
          // new chunk
        ...
        if (def_level >= level_info_.repeatedAncestorDefLevel) {
    ...
    if (prev_offset < num_levels) {
      chunks.push_back(
          {prev_offset, prev_value_offset, num_levels - prev_offset});
    }
#ifndef NDEBUG
  ...
  template <int kByteWidth>
  std::vector<Chunk> CalculateFixedWidth(
      const int16_t* def_levels,
      const int16_t* rep_levels,
      int64_t num_levels,
  ...
  std::vector<Chunk> CalculateBinaryLike(
      const int16_t* def_levels,
      const int16_t* rep_levels,
      int64_t num_levels,
      const ::arrow::Array& values) {
    ...
    auto handle_type = [&](auto&& type) -> std::vector<Chunk> {
      ...
      if constexpr (ArrowType::type_id == ::arrow::Type::NA) {
        ...
        return Calculate(def_levels, rep_levels, num_levels, [&](int64_t i) {
          ...
          ArrowType::type_id == ::arrow::Type::FIXED_SIZE_BINARY) {
        const auto& array =
            static_cast<const ::arrow::FixedSizeBinaryArray&>(values);
        const auto byte_width = array.byte_width();
        return Calculate(def_levels, rep_levels, num_levels, [&](int64_t i) {
          Roll(array.GetValue(i), byte_width);
        });
        ...
        return CalculateFixedWidth<sizeof(c_type)>(
            def_levels, rep_levels, num_levels, values);
      } else if constexpr (::arrow::is_decimal(ArrowType::type_id)) {
        return CalculateFixedWidth<ArrowType::kByteWidth>(
            def_levels, rep_levels, num_levels, values);
      } else if constexpr (::arrow::is_binary_like(ArrowType::type_id)) {
        return CalculateBinaryLike<::arrow::BinaryArray>(
            def_levels, rep_levels, num_levels, values);
      } else if constexpr (::arrow::is_large_binary_like(ArrowType::type_id)) {
        return CalculateBinaryLike<::arrow::LargeBinaryArray>(
            def_levels, rep_levels, num_levels, values);
        ...
        throw ParquetException(
            "Unsupported Arrow array type " + values.type()->ToString());
      }
    };
 ...
 private:
  ...
  // Minimum chunk size in bytes, the rolling hash will not be updated until
  // this size is reached for each chunk. Note that all data sent through the
  // hash function is counted towards the chunk size, including definition and
  // repetition levels.
  const int64_t min_chunk_size_;
  const int64_t max_chunk_size_;
  // The mask to match the rolling hash against to determine if a new chunk
  // should be created. The mask is calculated based on min/max chunk size and
  // the normalization level.
  const uint64_t rolling_hash_mask_;

  // Whether the rolling hash has matched the mask since the last chunk
  // creation. This flag is set true by the Roll() function when the mask is
  // matched and reset to false by NeedNewChunk() method.
  bool has_matched_ = false;
  // The current run of the rolling hash, used to normalize the chunk size
  // distribution by requiring multiple consecutive matches to create a new
  // chunk.
  int8_t nth_run_ = 0;
  // Current chunk size in bytes, reset to 0 when a new chunk is created.
  int64_t chunk_size_ = 0;
  // Rolling hash state, never reset only initialized once for the entire
  // column.
  uint64_t rolling_hash_ = 0;
};

ContentDefinedChunker::ContentDefinedChunker(
    const LevelInfo& level_info,
    int64_t min_chunk_size,
    int64_t max_chunk_size,
    int norm_level)
...
std::vector<Chunk> ContentDefinedChunker::GetChunks(
    const int16_t* def_levels,
    const int16_t* rep_levels,
    int64_t num_levels,
    const ::arrow::Array& values) {
